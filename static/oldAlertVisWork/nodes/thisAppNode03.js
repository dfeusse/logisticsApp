function node03() {
	...
	var arc = d3.svg.arc()
		.outerRadius(min/2 * 0.4)
		.innerRadius(min/2 * 0.3);
		
	var svg = d3.select("#third-magnitude-node").append("svg")
		.attr("width", width)
		.attr("height", height)
	var svgG = svg.append("g")
		.attr("transform", "translate(" + width/2 + "," + height/2 + ")");

	d3.csv("static/oldAlertVisWork/nodes/data03.csv", function(error, data) {
		...
		var circle = g.selectAll('circle.middlepart')
			...
			.attr('r', function(d) {return min/2 * 0.27})
			//.attr('fill', function(d) {return fillScale(d.value)})
			.attr('fill', '#fafafa')
			.attr('opacity', .1) //set opacity scale from 0.1 to 0.9
